    ...
    <div className="app-shell bg-gray-50 min-h-dvh pb-24">
      ...
      <div className="px-5 mt-4">
        ...
        {activeTab === tx.tabs[1] && (
          <div className="flex flex-col gap-3">
            {[
              { title: { KOR: '서울 언어교환 모임', ENG: 'Seoul Language Exchange' }, date: '2025년 4월 10일', emoji: '🗣️' },
              { title: { KOR: '한강 피크닉 클럽', ENG: 'Han River Picnic Club' }, date: '2025년 3월 22일', emoji: '🌸' },
              { title: { KOR: '홍대 보드게임 나이트', ENG: 'Hongdae Board Game Night' }, date: '2025년 3월 5일', emoji: '🎲' },
            ].map(m => (
              <div key={m.title.KOR} className="bg-white rounded-2xl p-4 flex items-center gap-4 border border-gray-100">
        ...
        {activeTab === tx.tabs[2] && (
          <div className="flex flex-col gap-3">
            {[{ title: { KOR: 'Seoul Crypto Meetup', ENG: 'Seoul Crypto Meetup' }, date: '2025년 5월 10일', members: '0/20', emoji: '💜' }].map(m => (
              <div key={m.title.KOR} className="bg-white rounded-2xl p-4 flex items-center gap-4 border border-gray-100">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                  style={{ background: 'linear-gradient(135deg, #EDE9FE, #C4B5FD)' }}>
                  {m.emoji}
                ...
                <div className="flex-1">
                  <p className="text-[13px] font-bold text-[#232129]">{m.title[lang]}</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">{m.date} · {m.members}</p>
                </div>
